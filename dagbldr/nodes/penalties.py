...
def bernoulli_and_correlated_log_gaussian_mixture_cost(
    ...
    z1 = ((true_1 - mu_1) ** 2) / tensor.exp(2 * log_sigma_1)
    z2 = ((true_2 - mu_2) ** 2) / tensor.exp(2 * log_sigma_2)
...
def bernoulli_and_correlated_gaussian_mixture_cost(
    binary_values, coeff_values, mu_values, sigma_values, corr_values,
    true_values):
    """
    Bernoulli combined with correlated gaussian mixture model negative log
    likelihood compared to true_values.
    ...
    corr_values = _subslice(corr_values, 0)
    inner1 = (0.5 * tensor.log(1 - corr_values ** 2) +
              tensor.log(sigma_1) + tensor.log(sigma_2) + tensor.log(
                  2 * np.pi))

    z1 = ((true_1 - mu_1) / sigma_1) ** 2
    z2 = ((true_2 - mu_2) / sigma_2) ** 2
    zr = (2. * corr_values * (true_1 - mu_1) * (true_2 - mu_2)) / (
        sigma_1 * sigma_2)
    z = z1 + z2 - zr
