...
ur4pe ur4pi urs5er ur5tes ur3the urti4 ur4tie u3ru 2us u5sad u5saac3u vag4 va4ge va5lie val5o val1u va5mo va5niz va5pi var5ied 3vat 4ve.
...
le3es ng1ho ng1spr nk3rup n5less 5noc3er1os nom1a6l
...
# Modified from Ned Batchelder https://nedbatchelder.com/code/modules/hyphenate.py
...
def rule_g2p(string, hyphenate=True):
    ...
    final_phones = []
    final_rules = []
    hyphenated = [hyphenate_word(ss) for ss in string.split(" ")]
    hyphenated = [h for hy in hyphenated for h in hy]
    pos_to_hyphenated = {}
    pos = 0
    cur_sub = 0
    cur_pos = 0
    while pos < len(string):
        pos_to_hyphenated[pos] = (hyphenated[cur_sub], cur_sub)
        if string[pos] != " ":
            if cur_pos >= (len(hyphenated[cur_sub]) - 1) or hyphenated[cur_sub][cur_pos] != string[pos]:
               cur_sub += 1
               cur_pos = 0
            else:
               cur_pos += 1
    ...
    while pos < len(string):
        ...
        for f in function_list:
            ...
            if r != False:
                ...
                rule = r[2]
                phones = r[3]
                for pp in range(pos, pos + advance_pos):
                    k = pp
                    pos_to_phones[k] = phones
                    pos_to_rules[k] = rule
    ...
    truly_final_phones = [final_phones[0]]
    truly_final_rules = [final_rules[0]]
    last_phone = final_phones[0]
    last_rule = final_rules[0]
    group = 0
    for p in sorted(pos_to_phones.keys()):
        r1 = pos_to_hyphenated[p]
        r2 = pos_to_phones[p]
        r3 = pos_to_rules[p]
        new_group = r1[1]
        if new_group != group:
            truly_final_phones.append("-")
            truly_final_rules.append("-")
            group = new_group
        if last_phone != r2:
           truly_final_phones.append(r2)
           truly_final_rules.append(r3)
           last_phone = r2
           last_rule = r3
    abs_final_phones = []
    abs_final_rules = []
    for a, b in zip(range(0, len(truly_final_phones) - 1), range(1, len(truly_final_phones))):
        if truly_final_phones[a] == "-" and truly_final_phones[b] == "< >":
            continue
        abs_final_phones.append(truly_final_phones[a])
        abs_final_rules.append(truly_final_rules[a])
    if truly_final_phones[-1] != "-":
        abs_final_phones.append(truly_final_phones[-1])
    ...
    for li in word_split:
        ...
        if li in cmu and not force_rule:
            ...
            try:
                ...
                print("Key Error: {}".format(li))
                li = remove_non_ascii(li).upper()
                if li[0] == "'":
                    li = li[1:]
                ...
                if li[-1] == "-":
                    li = li[:-1]
                li = li.replace('"', "")
                if len(li) == 0 or str(list(set(li))) == " ":
                    continue
...
if __name__ == "__main__":
    ...
    tst = "HOW CAN SHE SLAP!"
    tst = "HELLO FRIEND, IT IS NICE TO GREET YOURSELVES."
