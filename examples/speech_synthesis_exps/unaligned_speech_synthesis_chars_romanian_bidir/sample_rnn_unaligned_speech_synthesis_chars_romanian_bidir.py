...
filedir = "/Tmp/kastner/romanian_speakers/norm_info/"
...
filedir = "/Tmp/kastner/romanian_speakers/numpy_features/"
if filedir[-1] != "/":
    fd = filedir + "/"
else:
    fd = filedir
...
files = [f for f in files if "train" in f.split("/")[-1]]
'''

supertest = "supertest/numpy_features/"
files = sorted([supertest + f for f in os.listdir(supertest) if ".bak" not in f])
