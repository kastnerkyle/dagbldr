...
filedir = "/Tmp/kastner/bangla_speakers/norm_info/"
...
filedir = "/Tmp/kastner/bangla_speakers/numpy_features/"
#if not os.path.exists(filedir):
if filedir[-1] != "/":
    fd = filedir + "/"
else:
...
id_set = sorted(list(set([f.split("/")[-1].split("_")[1] for f in files])))
"""
id_set = ['shyamoshree',
           'chan',
           'moy',
...
id_lookup = {k: v for k, v in zip(id_set, range(len(id_set)))}

final_files = []
final_ids = []
for f in files:
    id_tag = f.split("/")[-1].split("_")[1]
    final_files.append(f)
    final_ids.append(id_lookup[id_tag])
files = final_files
file_ids = final_ids
