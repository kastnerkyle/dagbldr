...
norm_info_file = "/Tmp/kastner/romanian_multi_speakers/norm_info/"
...
sample_filedir = "/Tmp/kastner/romanian_multi_speakers/text_to_sample/"
...
if args.sample_text is not None:
    ...
    # Get sample text from the test set
    test_text = "/Tmp/kastner/romanian_speech/testing/text/"
    fetch_from_marge(test_text)
    with open(test_text + "SUS.txt", "r") as f:
        lines = f.readlines()
    lines = [" ".join(l.split(" ")[1:]).strip() for l in lines]
    lines = lines[:minibatch_size]
